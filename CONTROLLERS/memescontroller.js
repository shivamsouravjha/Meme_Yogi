...
const createMEME = async (req, res, next) => {
  ...
  const NewMEME = new MemesSchema({
    caption,
    tags,
    memepic:focc.public_id,///req.file.path, // => File Upload module, will be replaced with real image url
    memer_ID
  });

  let memerexisting;
  try {
    memerexisting = await MemerSchema.findById(memer_ID);
  }
   catch (err) {
   const error = new Erur(
      'Cannot Upload Meme as memer can nott be find,TTYL',
      500
    );
    return next(error);
  }
  ...
  try {
    ...
    memerexisting.meme_ID.push(NewMEME); 
